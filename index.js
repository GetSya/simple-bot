...
const autos = JSON.parse(fs.readFileSync('./database/autos.json'))
...
module.exports = syaa = async (syaa, mek) => {
  try {
   ...
   const isAutos = isGroup ? autos.includes(from) : false
    ...
    mess = {
      ...
      autos: "FITUR AUTO STICKER BELUM DI AKTIFKAN, AKTIFKAN TERLEBIH DAHULU KETIK /nsfw 1",
...
◩ *© Jojo-Bot* ◩`
  sendButMessage(from, menu2, `By @sofunsyabi.id`, [
        ...
        case 'sc':
          ...
          break
case 'cecan':
        let jingan = await fetchJson(
          `https://docs-jojoapi.herokuapp.com/api/cewe/indonesia?apikey=Syaa`
        );
        gmbr = jingan.result[Math.floor(Math.random() * jingan.result.url)];
        fs.writeFileSync(`./${sender}.jpeg`, await getBuffer(jingan.result.url));
        buttons = [
          {
            buttonId: `${command}`,
        ...
        buttonsMessage = {
          ...
          contentText: `Cantik Banget pacar gua`,
          buttons,
          headerType: 4,
        };
...
case 'neko':
  ...
  if (!isNsfw)return reply(mess.nsfw)
        let neko = await fetchJson(
          `https://docs-jojoapi.herokuapp.com/api/nsfw/nsfwNeko?apikey=Syaa`
        );
...
case 'hentai':
  ...
  if (!isNsfw)return reply(mess.nsfw)
        let wipu = await fetchJson(
          `https://docs-jojoapi.herokuapp.com/api/nsfw/${command}?apikey=Syaa`
        );
...
case 'autosticker':
  case 'autostiker':
    case 'autostik':
					...
					if (args.length < 1) return reply('Ketik 1 Untuk mengaktifkan')
					if (Number(args[0]) === 1) {
						if (isAutos) return reply(' *sudah aktif*  !!')
						autos.push(from)
						fs.writeFileSync('./database/autos.json', JSON.stringify(nsfw))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ MENGAKTIFKAN AUTO STICKER DI GROUP INI')
					} else if (Number(args[0]) === 0) {
						autos.splice(from, 1)
						fs.writeFileSync('./database/autos.json', JSON.stringify(nsfw))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ MENONAKTIFKAN AUTO STICKER DI GROUP INI')
					} else {
						reply(mess.tmbh)
					}
				break
...
case 'ssweb':
  ...
  capti = `Sikirinshot`
       ini_url = args.join(" ")
					...
					syaa.sendMessage(from, atetepe, image, { quoted: mek, caption: capti })
					break
case 'nulis':
        case 'tulis':
...
case 'translate':
  case 'dream':
  arasya = args.join(" ") 
                anu = await fetchJson(`https://docs-jojoapi.herokuapp.com/api/translate?kata=${arasya}&apikey=Syaa`)
                tr = `Kata : ${arasya}\nIndonesia : ${anu.result.text}\n\nFrom : ${anu.result.from}\nTo : ${anu.result.to}`
...
sendButMessage(from, sewa, `https://docs-jojoapi.herokuapp.com`, [
            {
              buttonId: `menu`,
              buttonText: {
...
case 'grupjojo':
  ...
  break
      case "igstalk":
        ...
        case 'islam':
          case 'quotesislami':
            anu = await fetchJson(`https://docs-jojoapi.herokuapp.com/api/randomquote/muslim?apikey=Syaa`)
...
case 'quote':
          case `quotes`:
            anu = await fetchJson(`https://docs-jojoapi.herokuapp.com/api/randomquote?apikey=Syaa`)
...
case 'bc':  
        case 'broadcast':  
                   if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					for (let _ of anu) {
						syaa.sendMessage(_.jid, buff, image, {quoted:mek ,caption: `「  *BROADCAST* 」\n\n${ini_bc}\n\nOwner : @${dtod.split('@')[0]}`})
					}
					reply('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					syaa.sendMessage(_.jid, `*[ BROADCAST JOJO ]* \n\n${ini_bc}\n\nOwner : @${dtod.split('@')[0]}`, text, { contextInfo: { mentionedJid: [dtod, sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					reply('```Success Broadcast```')
					}
					break
